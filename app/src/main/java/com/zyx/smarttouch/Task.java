...
public class Task {
    ...
    public void copySnsDB() throws Throwable {
        ...
        String sDir =ReplaceVideoActivity.getWxDir();
        String sDes = destDir + "/SnsMicroMsg.db";
        File file = new File(sDes);
        if(file.exists())
        {
            file.delete();
        }
...
//        outputStream.writeBytes("cd " + dataDir + "/data/" + Config.WECHAT_PACKAGE + "/MicroMsg/"+sDir+"\n");
...
//        outputStream.writeBytes("exit\n");
        ...
        String sSrc = dataDir + "/data/" + Config.WECHAT_PACKAGE + "/MicroMsg/"+sDir;
        Log.i("B4A","fffffsdfsdfsdfsdfsdf sSrc:"+sSrc);

        //outputStream.writeBytes("chmod 777 " +  dataDir);
        outputStream.writeBytes("cd " + dataDir + "/data/" + Config.WECHAT_PACKAGE + "/MicroMsg/"+sDir+"\n");
        outputStream.writeBytes("cat SnsMicroMsg.db> "+ sDes+"\n");
        //outputStream.writeBytes("cat SnsMicroMsg.db> /sdcard/SnsMicroMsg.db \n");
        outputStream.writeBytes("exit\n");
    ...
    public static void saveToJSONFile(ArrayList<SnsInfo> snsList, String fileName, boolean onlySelected) {
        ...
        for (int snsIndex=0; snsIndex<snsList.size(); snsIndex++) {
            ...
            String sType = EntryActivity.getContent(currentSns.rawXML,"<contentStyle><![CDATA[","]]></contentStyle>");
            if(sType !=null)
            {
                try {
                    currentSns.iType = Integer.parseInt(sType);
                }catch (NumberFormatException e)
                {
                    e.printStackTrace();
                }
            }
            currentSns.title = EntryActivity.getContent(currentSns.rawXML,"<title><![CDATA[","]]></title>");
            currentSns.contentUrl = EntryActivity.getContent(currentSns.rawXML,"<contentUrl><![CDATA[","]]></contentUrl>");
